...
#include <stackchan/avatar/skins/skin.h>
#include <esp_log.h>
#include <esp_err.h>
...
void StackChanAvatarDisplay::SetupUI()
{
    ...
    auto avatar = std::make_unique<ActiveAvatar>();  // Kconfig-selected skin (goudan)
    ...
    // 待机生命感: 瞳孔漂移+嘴部微动 (眨眼已由 BlinkModifier 负责, 关掉本模块眨眼通道)
    idle_life_modifier_id_ = stackchan.addModifier(std::make_unique<IdleLifeModifier>(/*enableBlink=*/false));
...
void StackChanAvatarDisplay::SetStatus(const char* status)
{
    ...
    if (strcmp(status, Lang::Strings::LISTENING) == 0) {
        ...
        is_idle = true;
        if (auto* il = static_cast<IdleLifeModifier*>(stackchan.getModifier(idle_life_modifier_id_)))
            il->set_speaking(false);
